...
module.exports = class PetController {
    static async create(req, res) {
        ...
        //Pets will always be available when created
        ...
        if (!images || images.length == 0) {
            res.status(422).json({ message: 'É necessário inserir pelo menos uma imagem' })
            return
        }
    ...
    static async schedule(req, res) {
        ...
        if (pet.user._id.toString() === user._id.toString()) {
            res.status(422).json({ message: 'Você não pode agendar uma visita ao seu próprio Pet!' })
            return
        }
        if (pet.adopter) {
        ...
        res.status(200).json({
            message: `A visita foi agenda com sucesso. Entre em contato com o ${pet.user.name} através do telefone: ${pet.user.phone}.`
        })
    }
    ...
    static async concludeAdoption(req, res) {
        ...
        const user = await getUserByToken(token)
        if (pet.user._id.toString() !== user._id.toString()) {
            res.status(422).json({ message: 'Ops... parece que esse pet não é seu!' })
            return
        }
        pet.available = false
        await Pet.findByIdAndUpdate(id, pet)
